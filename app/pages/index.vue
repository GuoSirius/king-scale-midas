...
<template>
  <div class="space-y-8">
    ...
    <!-- 品牌释义 -->
    <section class="glass rounded-3xl p-6 md:p-8 space-y-4">
      <h2 class="text-xl font-700">关于「金鳞·点石」</h2>
      <p class="text-white/60">{{ BRAND.purpose }}</p>
      <div class="grid md:grid-cols-2 gap-4">
        <div v-for="m in BRAND.meanings" :key="m.key" class="card">
          <div class="text-gold font-700 mb-1">{{ m.key }}</div>
          <div class="text-white/50 text-sm italic mb-2">「{{ m.quote }}」</div>
          <div class="text-white/70 text-sm leading-relaxed">{{ m.text }}</div>
        </div>
      </div>
      <p class="text-white/40 text-xs">{{ BRAND.disclaimer }}</p>
    </section>
