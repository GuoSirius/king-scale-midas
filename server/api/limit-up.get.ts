...
export default defineEventHandler(async (event) => {
  ...
  const conditions: (SQL | undefined)[] = [eq(limitRecords.limitType, 'up')]
  ...
  const where = and(...conditions) ?? eq(limitRecords.limitType, 'up')
  ...
  return { total: Number(tot?.c ?? 0), rows, filters: { date, board, minZt, kw } }
})
